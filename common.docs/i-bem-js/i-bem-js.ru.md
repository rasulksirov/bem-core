...
Можно создавать цепочки наследования – блок наследуется от другого, который, в свою очередь, наследуется от третьего и т.д.
...
При наследовании от блоков, предоставляющих сложную логику, удобнее сохранять основную функциональность базового блока. Новый блок, при этом, может реализоваывать собственную логику или переопределять методы базового. Такая схема наследования часто используется при работе с библиотечными блоками.

Чтобы доопределить существующий блок нужно создать новый блок с тем же именем. Пересекающиеся методы и модификаторы будут переопределены в соответствие с декларацией нового блока.
...
В соответствие с БЭМ-методологией состояния блока должны описываться модификаторами. По этому расширение функциональности блока зачастую предполагает реализацию поддержки новых модификаторов.

Для добавления модификатора нужно передать методу `decl` доопределяемого блока:
* хеш с ключами `modName` и `modVal`. Значением поля `modName` служит строка – имя модификатора. Значением `modVal` – строка со значением модификатора.
* хеш с реализацией методов, которые будут доступны для блока при установке соответствующего модификатора. Пересекающиеся методы будут переопределены методами из хеша.
