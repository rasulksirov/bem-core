...
Доступный набор функций может быть дополнен пользователем. Для этого нужно расширить прототип `BEMContext` новым статическим методом:
...
После этого добавленный метод будет доступен в шаблонах в контексте блока.
